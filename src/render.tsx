...
export function useState<T>(
  ...
  actions.forEach((action: any) => {
    if (typeof action === "function") state = action(state);
    else state = action;
  });
...
export function useEffect<T>(cb: () => void | (() => void), deps: Array<T>) {
...
function commitDeletion(fiber: Fiber) {
  ...
  if (fiber.dom) {
    ...
    if (
      domParentFiber?.dom instanceof HTMLElement ||
...
function updateDom(
  dom: FWDom,
  prevProps: FWProps | undefined,
  nextProps: FWProps | undefined
): void {
  // text nodes
...
function depsChanged(
  odeps: Array<any> | undefined,
  ndeps: Array<any> | undefined
) {
  if (!odeps || !ndeps) return true;
  if (odeps == ndeps) return false;

  if (odeps?.length !== ndeps?.length) return true;

  for (let i = 0; i < odeps.length; i++) {
    if (odeps[i] !== ndeps[i]) return true;
  }
  return false;
}
  ...
  hooks.forEach((hook: Hook, indx: number) => {
    if (hook.type == "effect") {
      ...
      let oldEffect = oldHooks?.[indx]
        ? (oldHooks?.[indx] as UseEffectHook)
        : undefined;
      ...
      if (depsChanged(deps, oldDeps)) {
        ...
        if (typeof cleanUp === "function") effectHook.cleanup = cleanUp;
        else effectHook.cleanup = undefined;
      }
    }
  });
}
...
function reconcileChildren(wipFiber: Fiber, children: Array<FWElement>): void {
  ...
  while (i < children.length || oldFiber) {
    ...
    if (i === 0) wipFiber.child = newFiber || undefined;
    else if (prevSibling) prevSibling.sibling = newFiber || undefined;
...
function performUnitOfWork(fiber: Fiber): Fiber | null {
  ...
  if (isFncComponent) updateFncComponent(fiber);
  else if (fiber.type != undefined) updateHostComponent(fiber);

  if (fiber.child) return fiber.child;
  // try siblings now
  let currFib: Fiber | undefined = fiber;
  while (currFib) {
    if (currFib.sibling) return currFib.sibling;
...
function commitWork(fiber: Fiber | undefined): void {
  ...
  if (dontRunRelatives) return; // stoping here, deleting shouldnt be recursive unlike placing and updating
...
export function render(element: FWElement, container: HTMLElement): void {
  wipRoot = {
    type: "ROOT",
    dom: container,
    props: {
