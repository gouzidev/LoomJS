import {
  EffectTag,
  FWElement,
  FWProps,
  Fiber,
  eventCallBack,
  Hook,
  log,
  SupportedElement,
  SVG_CAMEL_CASE_ATTRS,
} from "./types.js";

import { camelToKebab, isEventListener, isProperty } from "./helper.js";

function addEvent(node: SupportedElement, prop: string, value: eventCallBack) {
  switch (prop) {
    case "onHover":
      node.addEventListener("mouseover", value as eventCallBack);
      break;
    case "onFocus":
      node.addEventListener("focus", value as eventCallBack);
      break;
    default:
      const name = prop.toLowerCase().slice(2);
      node.addEventListener(name, value as eventCallBack);
  }
}

function removeEvent(
  node: SupportedElement,
  prop: string,
  value: eventCallBack
) {
  switch (prop) {
    case "onHover":
      node.removeEventListener("mouseover", value as eventCallBack);
      break;
    case "onFocus":
      node.removeEventListener("focus", value as eventCallBack);
      break;
    default:
      const name = prop.toLowerCase().slice(2);
      node.removeEventListener(name, value as eventCallBack);
  }
}

function FWremAttr(
  node: SupportedElement,
  prop: string,
  value: eventCallBack | string
): void {
  if (prop == "className" && typeof value == "string") node.classList = "";
  else if (["checked", "selected", "disabled"].includes(prop)) {
    let inputEl = node as HTMLInputElement;
    switch (prop) {
      case "checked":
        inputEl.checked = false;
        inputEl.removeAttribute("checked");
        break;
      case "selected":
        inputEl.removeAttribute("selected");
        break;
      case "disabled":
        inputEl.disabled = false;
        inputEl.removeAttribute("disabled");
        break;
    }
  }
  // event
  else if (isEventListener(prop)) {
    if (typeof value != "string")
      // not str -> eventCB
      removeEvent(node, prop, value);
  } else if (isProperty(prop)) {
    if (prop == "nodeValue") {
      node.nodeValue = null;
    } else node.removeAttribute(prop);
  }
}

function FWsetAttr(
  node: SupportedElement,
  prop: string,
  value: string | eventCallBack
): void {
  if (prop === "children") return;

  if (prop == "className") {
    if (!value) return;
    let classes = (value as string).split(" ");
    classes.forEach((cls) =>
      cls.trim() != "" ? node.classList.add(cls.trim()) : null
    );
  } else if (["checked", "selected", "disabled"].includes(prop)) {
    let inputEl = node as HTMLInputElement;
    switch (prop) {
      case "checked":
        inputEl.checked = value === "true";
        break;
      case "selected":
        inputEl.setAttribute("selected", value as string);
        break;
      case "disabled":
        if (value == "false") {
          inputEl.disabled = false;
          inputEl.removeAttribute("disabled");
        } else {
          inputEl.disabled = true;
        }
        break;
    }
  }
  // event
  else if (isEventListener(prop)) {
    addEvent(node, prop, value as eventCallBack);
  } else {
    if (prop == "nodeValue") {
      node.nodeValue = value as string;
    } else {
      const isSvg = node instanceof SVGElement;
      const attrName =
        isSvg && !SVG_CAMEL_CASE_ATTRS.has(prop) ? camelToKebab(prop) : prop;
      node.setAttribute(attrName, value as string);
    }
  }
}

export { FWremAttr, FWsetAttr, addEvent, removeEvent };
